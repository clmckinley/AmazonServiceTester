...
namespace Sensus.DataStores.Remote
{
    ...
    /// 
    /// </summary>
    public class AmazonS3RemoteDataStore
    {
        ...
        [JsonIgnore]
        public bool CanRetrieveWrittenData
        {
            get
            {
                return true;
        ...
        public Task WriteDataStreamAsync(Stream stream, string name, string contentType, CancellationToken cancellationToken)
        {
            return Task.Run(async () =>
            {
                ...
                try
                {
                    ...
                    await Put(s3, stream, DATA_DIRECTORY + "/" + (string.IsNullOrWhiteSpace(_folder) ? "" : _folder + "/") + (string.IsNullOrWhiteSpace(ParticipantId) ? "" : ParticipantId + "/") + name, contentType, cancellationToken);
                }
                finally
                {
                    DisposeS3(s3);
        ...
        public Task WriteStringAsync(string val, CancellationToken cancellationToken)
        {
            return Task.Run(async () =>
            {
                AmazonS3Client s3 = null;
        ...
        public string GetDatumKey(string str)
        {
            return DATA_DIRECTORY + "/" + (string.IsNullOrWhiteSpace(_folder) ? "" : _folder + "/") + (string.IsNullOrWhiteSpace(ParticipantId) ? "" : ParticipantId + "/") + "ConsoleTest" + "/" + str + ".json";
        }

        public async Task<string> GetDatumAsync(string datumKey, CancellationToken cancellationToken)
        {
            AmazonS3Client s3 = null;
            string rString = null;
